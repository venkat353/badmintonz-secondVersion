...
const LoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We keep your existing logic to extract the role from the token
      let roleName = "USER"; // Default fallback
      
      const decoded = parseJwt(token);
      if (decoded && decoded.role) {
        roleName = decoded.role.replace("ROLE_", "");
      ...
      // 3. USE CONTEXT TO LOGIN (Instead of manual localStorage)
      // We pass the token, the calculated role, and the email user typed
      login(token, roleName, email);
